...
This repository contains `STR_regression.R`, code to perform association testing of STRs with built-in parallelization. The code is written in R and can be found in the scripts folder. The code uses built-in parallelization for improved performance with `--threads`.
