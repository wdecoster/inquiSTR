...
# TODO: check if size_select is only MEAN, MIN or MAX, if not throw early error
...
assoc_binary <- function(arg, calls_file, phenotype, no_cols, covariates) {
    ...
    results_calls_file_selected <- as.data.frame(matrix(0, 1, 13))
    colnames(results_calls_file_selected) <- c("VariantID", "OR", "OR_L95", "OR_U95", "OR_stdErr", "Pvalue", "N", "AvgSize", "Group1_AvgSize", "Group2_AvgSize", "Group2_Group1_absAvgSizeDiff", "Group2_Group1_OR_for_absAvgSizeDiff", "model")
    for (i in seq(no_cols, ncol(calls_file_selected), 1)) {
        ...
        tabular_result <- as.data.frame(cbind(Predictors, OR, OR_L95, OR_U95, OR_stdErr, Pvalue, N, AvgSize, Group1_AvgSize, Group2_AvgSize, Group2_Group1_absAvgSizeDiff, Group2_Group1_OR_for_absAvgSizeDiff, model))
        ...
        progress(i, max.value = ncol(calls_file_selected), init = TRUE, progress.bar = FALSE, console = TRUE, gui = FALSE)
    }
...
assoc_continuous <- function(arg, calls_file, phenotype, no_cols, covariates) {
    ...
    for (i in seq(no_cols, ncol(calls_file), 1)) {
        ...
        progress(i, max.value = ncol(calls_file_selected), init = (value == 1), progress.bar = TRUE, consolute = TRUE, gui = FALSE)
    }
...
prepare_calls <- function(calls, sample_list_wPheno, size_select, missing_cutoff) {
    if (size_select == "MEAN") {
        calls2 <- (pmax(calls$H1, calls$H2,na.rm=TRUE) + pmin(inqH1[,2],inqH2[,2],na.rm=TRUE)) / 2
    } else if (size_select == "MAX") {
        calls2 <- pmax(calls$H1, calls$H2, na.rm = TRUE)
    } else if (size_select == "MIN") {
...
parse_arguments <- function() {
    p <- arg_parser("Run association testing for STRs with different modes and options. Version 1.4, October 21, 2022")
    p <- add_argument(p, "--input", help = "Input STR file with a header, first 3 columns are chrom, begin, end, and rest are sample ids with inqH1 & inqH2 STR lengths", type = "character", nargs = 1, required = TRUE)
    ...
    p <- add_argument(p, "--size_select", help = "Choose a size_select name from following: MEAN, MAX, MIN; meaning H1+H2 alleles divided by two, maximum of two, or minimum of two. Missing alleles are not considered.", type = "character", nargs = 1)
    ...
    p <- add_argument(p, "--run", help = "Run mode, select among: full (entire file will be analyzed), chromosome (chromosome indicated by --chr will be analyzed), chr_interval (intervals between chr:begin:end defined by --chr --begin --end will be analyzed), bed_interval (an intersection with a bed file defined by --bed will be analyzed), single_variant (a single variant defined by --single_variant will be analyzed; can be combined with --expandedAllele)", type = "character", nargs = 1)
    ...
    p <- add_argument(p, "--expandedAllele", help = "A cut-off integer number to define 2 groups with and without expanded allele", type = "integer", nargs = "?")
    Version <- "Version 1.4, October 21, 2022"
    print(Version)
...
if ((!is.na(arg$chr) && !is.na(arg$chr_begin) && !is.na(arg$chr_end))) {
    ...
    calls <- read_calls(input = arg$input, chr = arg$chr)
}
