...
fn parse_bed_file(file_path: &PathBuf) -> Result<HashMap<String, TruthRecord>, Box<dyn std::error::Error>> {
    ...
    for line in reader.lines() {
        ...
        if fields.len() != 9 {
            eprintln!("Warning: Skipping malformed BED line {} (expected 9 fields, got {}): {}", 
                     line_count, fields.len(), line);
            continue;
        }
...
fn parse_inquistr_file(file_path: &PathBuf) -> Result<HashMap<String, InquiSTRRecord>, Box<dyn std::error::Error>> {
...
fn parse_vcf_file(file_path: &PathBuf) -> Result<HashMap<String, TruthRecord>, Box<dyn std::error::Error>> {
    let reader = crate::utils::reader(&file_path.to_string_lossy().to_string());
    let mut records = HashMap::new();
    ...
    for line in reader.lines() {
        ...
        let alt_lengths: Vec<f64> = alt_seqs.iter()
    ...
    println!("VCF variants processed: {} total, {} SNPs filtered out, {} STR variants retained", 
             total_variants, snp_variants, records.len());
...
fn select_allele(values: &[f64], mode: &str) -> Option<f64> {
    ...
    match mode.to_uppercase().as_str() {
        "MAX" => finite_values.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).copied(),
        "MIN" => finite_values.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).copied(),
...
pub fn benchmark(inquistr_file: PathBuf, vcf_file: Option<PathBuf>, bed_file: Option<PathBuf>, mode: String, plot_file: PathBuf) {
    ...
    let layout = Layout::new()
        .title(Title::with_text(format!("inquiSTR vs Truth Genotypes (Mode: {}, R² = {:.4})", mode, r_squared)))
