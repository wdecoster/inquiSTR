...
fn parse_combined_file(combined: &std::path::Path) -> (Array2<f64>, Vec<String>, Vec<String>) {
    ...
    if header_fields.len() < 5 || header_fields[0] != "chromosome" || header_fields[1] != "begin" || header_fields[2] != "end" {
    ...
    for i in 0..num_samples {
        ...
        if !h1_col.ends_with("_H1") {
            panic!("Invalid header: expected column {} to end with '_H1', got '{}'", 3 + i * 2, h1_col);
        }
        if !h2_col.ends_with("_H2") {
            panic!("Invalid header: expected column {} to end with '_H2', got '{}'", 3 + i * 2 + 1, h2_col);
        }
        ...
        if sample_name_h1 != sample_name_h2 {
            panic!("Header error: H1 and H2 columns have different sample names: '{}' vs '{}'", sample_name_h1, sample_name_h2);
        }
    ...
    for (region_idx, line) in data_lines.iter().enumerate() {
        ...
        if fields.len() != expected_cols {
            panic!(
                ...
                region_idx + 2, expected_cols, fields.len(), line
            );
        }
        ...
        for sample_idx in 0..num_samples {
            ...
            let h1_val: f64 = fields[h1_idx].parse()
                .map_err(|e| format!("Invalid H1 value '{}' at line {}, column {}: {}", 
                                   fields[h1_idx], region_idx + 2, h1_idx + 1, e))
                .unwrap_or_else(|e| {
            ...
            let h2_val: f64 = fields[h2_idx].parse()
                .map_err(|e| format!("Invalid H2 value '{}' at line {}, column {}: {}", 
                                   fields[h2_idx], region_idx + 2, h2_idx + 1, e))
                .unwrap_or_else(|e| {
                    // Handle NaN values specifically
...
fn perform_simple_pca(data: &Array2<f64>) -> (Array2<f64>, Array1<f64>) {
    ...
    let pc2_idx = if feature_variances.len() > 1 { feature_variances[1].0 } else { 0 };
    ...
    let var2 = if feature_variances.len() > 1 { feature_variances[1].1 } else { 0.0 };
...
fn create_pca_plot(
    ...
    let layout = Layout::new()
        ...
        .x_axis(
            PlotAxis::new().title(Title::with_text(format!(
                "PC1 ({:.1}% variance)",
    ...
    println!("  Total explained by PC1+PC2: {:.1}%", explained_variance[0] + explained_variance[1]);
}
